...
class Vision:
    ...
    def iter_visible_entities(self, cur_coords : tuple[float, float], entities : dict[Any, Any]) -> Generator[tuple[Any, tuple[float, float]], None, None]:
        for entity, entity_coords in entities.items():
            if self.abs_in_view(cur_coords, entity_coords):
                yield entity, entity_coords

    def dict_visible_entities(self, cur_coords : tuple[float, float], entities : dict[Any, Any]) -> dict[Any, tuple[float, float]]:
